...
const ChatBot: React.FC<ChatBotProps> = ({ isOpen, onClose, energyData }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const newMessages = [...messages, userMessage, result.result];
      setMessages(newMessages);
      
      // 保存聊天记录到localStorage
      const chatHistory = JSON.parse(localStorage.getItem('chatHistory') || '[]');
      const currentSession = chatHistory.find((session: any) => 
      ...
      if (currentSession) {
        currentSession.messages.push(
          ...
          { text: result.result, isUser: false }
        );
      } else {
        chatHistory.push({
      ...
      const newMessages = [...messages, userMessage, mockResponse.result];
      setMessages(newMessages);
      ...
      if (currentSession) {
        currentSession.messages.push(
          { text: userMessage, isUser: true },
          { text: mockResponse.result, isUser: false }
        );
      } else {
        chatHistory.push({
          timestamp: new Date().toISOString(),
